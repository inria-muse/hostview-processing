...
DB.prototype.insert = function (table, rowdata, cb) {
  if (!this._db) { return cb(new Error('No db connection [' + this.dburl + ']')) }
...
DB.prototype.select = function (table, filter, cb) {
  if (!this._db) { return cb(new Error('No db connection [' + this.dburl + ']')) }
  if (filter) { this._db.select('*').from(table).where(filter).rows(cb) } else { this._db.select('*').from(table).rows(cb) }
}
