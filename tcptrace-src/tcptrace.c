...
//	udptrace_init();
  ...
  //  if (run_continuously) {
    //  trace_init();
    //}
   //Oana--- create the timeseries files
   if (timeseries){
       static char myfilename[150];
       snprintf (myfilename, sizeof (myfilename), "%s/%s%s",output_file_dir, output_file_prefix, TSR_RTT_GRAPH_FILE_EXTENSION);
       printf("%s",myfilename);
       if ((myrttfile = fopen (myfilename, "w")) == NULL) {
    	    perror (myfilename);
       }
       snprintf (myfilename, sizeof (myfilename), "%s/%s%s",output_file_dir, output_file_prefix, TSR_THROUGHPUT_FILE_EXTENSION);
       printf("%s",myfilename);
       if ((mytputfile = fopen (myfilename, "w")) == NULL) {
    	    perror (myfilename);
       }
       snprintf (myfilename, sizeof (myfilename), "%s/%s%s",output_file_dir, output_file_prefix, TSR_PLOT_FILE_EXTENSION);
       printf("%s",myfilename);
       if ((mytsgfile = fopen (myfilename, "w")) == NULL) {
            perror (myfilename);
       }
       snprintf (myfilename, sizeof (myfilename), "%s/%s%s",output_file_dir, output_file_prefix, TSR_RAW_FILE_EXTENSION);
       printf("%s",myfilename);
       if ((myrawpktsfile = fopen (myfilename, "w")) == NULL) {
            perror (myfilename);
       }
       snprintf (myfilename, sizeof (myfilename), "%s/%s%s",output_file_dir, output_file_prefix, TSR_OWIN_FILE_EXTENSION);
       printf("%s",myfilename);
       if ((myowinfile = fopen (myfilename, "w")) == NULL) {
       	perror (myfilename);
       }
   }
   //---
