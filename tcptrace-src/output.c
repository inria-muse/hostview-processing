...
//Oana--- put timestamp in epoch format  and put usecs in the good format (instead of 2212 put 002212)
...
//       fprintf(stdout,"\telapsed time:  %s\n", elapsed2str(etime));
       fprintf(stdout,"\tfirst packet:  %ld.%.6ld\n", (long)ptp->first_time.tv_sec, (long)ptp->first_time.tv_usec );
       fprintf(stdout,"\tlast packet:  %ld.%.6ld\n",  (long)ptp->last_time.tv_sec, (long)ptp->last_time.tv_usec);
       fprintf(stdout,"\telapsed time:  %ld.%.6ld\n", (long) etime_secs, (long) etime_usecs);
//---
	...
	StatLineF("throughput","Bps","%8.4f",
		  (double) (pab->unique_bytes) / etime,
		  (double) (pba->unique_bytes) / etime);
    //Oana--- calculate throughput on data_bytes not unique_bytes
    StatLineF("throughput data","Bps","%8.4f",
            		(double) (pab->data_bytes) / etime,
            		(double) (pba->data_bytes) / etime);

    //Oana--- calculate statistics for throughput
    // we do all this operations here because the throughput is calculated online (for each packet that arrives)
    // so there might be at the end of the connection results that we didn't take into account

    if ((pab->thru_stat_min == 0) || (pab->thru_stat_min > pab->thru_stat_bytes))
    	pab->thru_stat_min = pab->thru_stat_bytes;

    if (pab->thru_stat_max < pab->thru_stat_bytes)
    	pab->thru_stat_max = pab->thru_stat_bytes;

    pab->thru_stat_sum += pab->thru_stat_bytes;
    pab->thru_stat_sum2 += pab->thru_stat_bytes * pab->thru_stat_bytes;

    struct  timeval difference;

    difference=tvSubtract1(ptp->last_time,pab->thru_stat_firsttime);
    fprintf(stdout,"DEBUG: remaining bytes %ld  difference %ld.%.6ld  sum %f\n",pab->thru_stat_bytes, difference.tv_sec, difference.tv_usec , pab->thru_stat_sum );
    //fprintf(stdout,"THRUDEBUG: %7.4f \n",pab->thru_debug);

    if (( (long)difference.tv_sec == 0) && ((long) difference.tv_usec== 0)){
    	fprintf(stdout,"XSTAT zero: diff %s %s %ld.%.6ld\n",  pab->ptp->a_endpoint, pab->ptp->b_endpoint, (long) difference.tv_sec, (long) difference.tv_usec);
    }
    else if (difference.tv_sec < 2)
    {
    	++pab->thru_stat_count;
    	fprintf(stdout,"XSTAT two: diff %s %s %ld.%.6ld %d\n",  pab->ptp->a_endpoint, pab->ptp->b_endpoint, (long) difference.tv_sec, (long) difference.tv_usec, pab->thru_stat_count);
    }
    else
    {
    	pab->thru_stat_min = 1;
    	pab->thru_stat_count += (long) difference.tv_sec;
    	fprintf(stdout,"XSTAT more: diff %s %s %ld.%.6ld\n",  pab->ptp->a_endpoint, pab->ptp->b_endpoint, (long) difference.tv_sec, (long) difference.tv_usec);
    }

    fprintf (stdout, "XSTAT OUT: %s %s %ld.%.6ld %ld %ld %ld %ld %ld %d\n",  pab->ptp->a_endpoint, pab->ptp->b_endpoint,  (long) pab->thru_stat_firsttime.tv_sec, (long) pab->thru_stat_firsttime.tv_usec, (long) pab->thru_stat_bytes, (long) pab->thru_stat_min, (long) pab->thru_stat_max, (long) pab->thru_stat_sum, (long) pab->thru_stat_sum2, (int) pab->thru_stat_count );

    if ((pba->thru_stat_min == 0) || (pba->thru_stat_min > pba->thru_stat_bytes))
    	pba->thru_stat_min = pba->thru_stat_bytes;

    if (pba->thru_stat_max < pba->thru_stat_bytes)
    	pba->thru_stat_max = pba->thru_stat_bytes;

    pba->thru_stat_sum += pba->thru_stat_bytes;
    pba->thru_stat_sum2 += pba->thru_stat_bytes * pba->thru_stat_bytes;

    difference=tvSubtract1(pba->ptp->last_time,pba->thru_stat_firsttime);
    fprintf(stdout,"DEBUG: remaining bytes %ld  difference %ld.%.6ld sum %f\n",pba->thru_stat_bytes, difference.tv_sec, difference.tv_usec, pab->thru_stat_sum  );
    //fprintf(stdout,"THRUDEBUG: %7.4f \n",pba->thru_debug);

    if (( (long)difference.tv_sec == 0) && ((long) difference.tv_usec== 0)){
       	fprintf(stdout,"XSTAT zero: diff %s %s %ld.%.6ld\n",  pba->ptp->a_endpoint, pba->ptp->b_endpoint, (long) difference.tv_sec, (long) difference.tv_usec);
       }
       else if (difference.tv_sec < 2)
       {
       	++pba->thru_stat_count;
       	fprintf(stdout,"XSTAT two: diff %s %s %ld.%.6ld %d\n",  pba->ptp->a_endpoint, pba->ptp->b_endpoint, (long) difference.tv_sec, (long) difference.tv_usec, pab->thru_stat_count);
       }
       else
       {
       	pba->thru_stat_min = 1;
       	pba->thru_stat_count += (long) difference.tv_sec;
       	fprintf(stdout,"XSTAT more: diff %s %s %ld.%.6ld %ld.%.6ld %ld.%.6ld\n",  pba->ptp->a_endpoint, pba->ptp->b_endpoint, (long) difference.tv_sec, (long) difference.tv_usec, (long) pba->thru_stat_firsttime.tv_sec, (long) pba->thru_stat_firsttime.tv_usec, pba->ptp->last_time.tv_sec, pba->ptp->last_time.tv_usec);
       }

    fprintf (stdout, "XSTAT OUT: %s %s %ld.%.6ld %ld %ld %ld %ld %ld %d\n",  pba->ptp->a_endpoint, pba->ptp->b_endpoint,  (long) pba->thru_stat_firsttime.tv_sec, (long) pba->thru_stat_firsttime.tv_usec, (long) pba->thru_stat_bytes, (long) pba->thru_stat_min, (long) pba->thru_stat_max, (long) pba->thru_stat_sum, (long) pba->thru_stat_sum2, (int) pba->thru_stat_count );




    StatLineI("thr samples","", pab->thru_stat_count, pba->thru_stat_count);
    StatLineF("thr min","Bps","%8.4f",
    		(double)pab->thru_stat_min,
    		(double)pba->thru_stat_min);
    StatLineF("thr max","Bps","%8.4f",
    		(double)pab->thru_stat_max,
    		(double)pba->thru_stat_max);
    StatLineF("thr avg","Bps","%8.4f",
    		Average(pab->thru_stat_sum, pab->thru_stat_count) ,
    		Average(pba->thru_stat_sum, pba->thru_stat_count) );
    StatLineF("thr stdev","Bps","%8.4f",
    		Stdev(pab->thru_stat_sum, pab->thru_stat_sum2, pab->thru_stat_count) ,
    		Stdev(pba->thru_stat_sum, pba->thru_stat_sum2, pba->thru_stat_count));
    //---
...
//       char first_usecs [50];
//       int len;
//       sprintf(USECS,"%s","000000");
//       sprintf(BUF,"%ld", (long)pup->first_time.tv_usec);
//       strcat(USECS,BUF);
//       len=strlen(USECS);
//       sprintf(first_usecs,"%c%c%c%c%c%c",USECS[len-6],USECS[len-5],USECS[len-4],USECS[len-3],USECS[len-2],USECS[len-1]);
       fprintf(stdout,"\tfirst packet:  %ld.%.6ld\n", (long)pup->first_time.tv_sec, (long)pup->first_time.tv_usec);


//       char last_usecs [50];
//       sprintf(USECS,"%s","000000");
//       sprintf(BUF,"%ld", (long)pup->last_time.tv_usec);
//       strcat(USECS,BUF);
//       len=strlen(USECS);
//       sprintf(last_usecs,"%c%c%c%c%c%c",USECS[len-6],USECS[len-5],USECS[len-4],USECS[len-3],USECS[len-2],USECS[len-1]);
       fprintf(stdout,"\tlast packet:  %ld.%.6ld\n",  (long)pup->last_time.tv_sec, (long)pup->last_time.tv_usec);

//       char elapsed_usecs [50];
//       sprintf(USECS,"%s","000000");
//       sprintf(BUF,"%d", etime_usecs);
//       strcat(USECS,BUF);
//       len=strlen(USECS);
////     fprintf(stdout,"asd %s -- %s", USECS,BUF);
//       sprintf(elapsed_usecs,"%c%c%c%c%c%c",USECS[len-6],USECS[len-5],USECS[len-4],USECS[len-3],USECS[len-2],USECS[len-1]);


       fprintf(stdout,"\telapsed time:  %ld.%.6ld\n", etime_secs, etime_usecs);
//---
