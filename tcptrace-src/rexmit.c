...
static void dump_rtt_sample (tcb *,
			     ...
			     double);
//Oana---
static void dump_myrtt_sample (tcb *,
			     segment *,
			     double);
//---
	   ...
	   Bool rexmit_prev)
{
    ...
    //Oana---
    if (timeseries && (pseg->retrans == 0)) {
	dump_myrtt_sample (ptcb, pseg, etime_rtt);
    }
    //---
		...
		double etime_rtt)
{
	...
	fprintf (myrttfile, "%s %s %s %lu %lu.%lu %f \n",
			flowid, name_from, name_to,
			pseg->seq_firstbyte, (long)current_time.tv_sec,(long)current_time.tv_usec,
			(double) (etime_rtt / 1000.0) /* convert from us to ms */ );
}
//---
