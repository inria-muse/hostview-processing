    ...
    struct tcphdr *ptcp)
{
    ...
    //Oana-- nothing to do here for my timeseries file
    ...
    void *plast)
{
    ...
    /* Setting the flowid */
    char flowid[50];
    snprintf(flowid,sizeof(flowid),"%s2%s", thisdir->host_letter, thisdir->ptwin->host_letter);
    ...
    //Oana--- test print to see what tcptrace is counting
    //fprintf(stdout,"ip length %d \n",ip_len);
    //fprintf(stdout,"tcp length %d \n",tcp_length);
    //fprintf(stdout,"tcp data length %d \n",tcp_data_length); /*as given by ipsumdump -L*/
    //---
...
%s->%s: rexmitted FIN had diff. seqnum! (was %lu, now %lu, etime: %d sec)\n",
    ...
    //Diego--- print per packet simple statistics
    if (timeseries) {
      int direction = (thisdir->host_letter > thisdir->ptwin->host_letter);

      char flowcode[50];
      if(direction==0){
          //SRC to DST
          snprintf(flowcode,sizeof(flowcode),"%s2%s", thisdir->host_letter, thisdir->ptwin->host_letter);
      } else {
          //DST to SRC
          snprintf(flowcode,sizeof(flowcode),"%s2%s", thisdir->ptwin->host_letter, thisdir->host_letter);
      }

      fprintf(myrawpktsfile,"%s,%d,%ld.%.6ld,%d,%d,%d\n", flowcode, direction, (long)current_time.tv_sec, (long)current_time.tv_usec, ip_len, tcp_length, tcp_data_length);
    }

    //Oana--- print the out-of-order packets
    if (out_order && timeseries && show_out_order) {
    	fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld OOO\n", flowid, name_from, name_to,(long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(thisdir,end));
    }
    //---
	...
	//Oana--- print retransmitted packets
	if (!(FIN_SET(ptcp)||SYN_SET(ptcp)) && timeseries && show_rexmit && !hw_dup) {
			fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld R\n", flowid, name_from, name_to, (long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(thisdir,end));
		}
	//---
    } else {
	thisdir->seq = end;
    ...
    if(probe) {
    	...
    	//Oana--- also add this in the timeseries file
    	if(timeseries && show_zwnd_probes){
    		fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld P\n", flowid, name_from, name_to, (long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(thisdir,end));
    	}
    	//---
    }
    ...
    //Oana --print R SYN and R FIN
    if (timeseries) {
    	if (SYN_SET(ptcp) && !hw_dup) {		/* SYN  */
    		/* if we're using time offsets from zero, it's easier if */
    		/* both graphs (a2b and b2a) start at the same point.  That */
    		/* will only happen if the "left-most" graphic is in the */
    		/* same place in both.  To make sure, mark the SYNs */
    		...
    		if (retrans_num_bytes>0) {
    			fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld R SYN\n", flowid, name_from, name_to,(long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(thisdir,start));
    		}
    		else {
    			fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld SYN\n", flowid, name_from, name_to, (long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(thisdir,start));
    		}
    	} else if (FIN_SET(ptcp) && !hw_dup) {	/* FIN  */
    		/* Wed Sep 18, 2002 - bugfix
    		 * Check if data is present in the last packet.
    		 * We will draw the data bytes with the normal color
    		 * and then change the color for the last byte of FIN.
    		...
    		if (retrans_num_bytes>0) {
    			fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld R FIN\n", flowid, name_from, name_to,(long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(thisdir,end));
    		}
    		else {
    			fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld FIN\n", flowid, name_from, name_to, (long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(thisdir,end));
    		}
    	}
    }
    //---
   ...
   if (tlinepl != NO_PLOTTER) {
	...
	//Oana--- add RSTs to the timeseries file
	if (timeseries) {
		fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld RST_IN\n", flowid, name_to, name_from,(long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(otherdir,plot_at));
		fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld RST_OUT\n", flowid, name_from, name_to,(long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(thisdir,start));
	}
	//---
	if (ACK_SET(ptcp))
	    ++thisdir->ack_pkts;
	...
	//Oana--- here we have the double ACK (ack not different than the previous one)
	if (timeseries && thisdir->time.tv_sec != -1) {
		if (thisdir->ack != ack) {

		} else {
			//change position of name_to to with name_from because tcptrace is plotting to the other file (to_tsgpl instead of from_tsgpl)
			fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld double ACK\n", flowid, name_to, name_from,(long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(otherdir,ack));
			if (show_triple_dupack && (ack_type == TRIPLE)) {
				fprintf(mytsgfile,"%s %s %s %ld.%.6ld %ld triple double ACK\n", flowid, name_to, name_from,(long)current_time.tv_sec, (long)current_time.tv_usec, SeqRep(otherdir,ack));
			}
		}
	}
	//---
...
/* 	thisdir->windowend = winend;  (moved above "only" point) */
	...
	if (thisdir->previous_owin_sample_time.tv_sec == 0) {
	  /* if this is first ever sample for thisdir */
		...
		if (timeseries) {
			fprintf(myowinfile,"%s %s %lu.%.6lu %llu wavg green\n", name_from, name_to,(long)thisdir->previous_owin_sample_time.tv_sec,(long)thisdir->previous_owin_sample_time.tv_usec,
		        	(total_elapsed_time)?((u_llong)((thisdir->owin_wavg)/total_elapsed_time)):0);
		}
	    	thisdir->previous_owin_sample_time = thisdir->last_time;
		thisdir->previous_owin_sample = owin;
    ...
    if (timeseries) {
    	fprintf(myowinfile,"%s %s %lu.%.6lu %lu owin red\n", name_from, name_to, (long)current_time.tv_sec, (long)current_time.tv_usec, owin);
    	if (show_rwinline) {
    		fprintf(myowinfile,"%s %s %lu.%.6lu %lu rwin yellow\n",name_from, name_to, (long)current_time.tv_sec,(long) current_time.tv_usec,
    				otherdir->win_last);
    	}
    	fprintf(myowinfile,"%s %s %lu.%.6lu %llu owin_avg blue\n", name_from, name_to, (long)current_time.tv_sec,(long) current_time.tv_usec,
    			(thisdir->owin_count?(thisdir->owin_tot/thisdir->owin_count):0));
    }
    }
    if (run_continuously) {
